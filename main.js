...
function calcScore(beer, foam) {
  ...
  return Math.round(close * fill * 1000);
}
...
function upd(dt) {
  ...
  if (ready && started) {
    ...
    document.getElementById("tv").textContent = remain.toFixed(1);
  ...
  if (hP && peff > 0.06 && started) {
    ...
    if (tot < GMAX) {
      const addBeer = Math.min(rate * (0.45 + ge * 0.65), GMAX - tot);
    ...
    if (t2 < GMAX) {
      const foamRate = (1 - ge) * peff * dt * 18 + impact * dt * 12;

      foam += Math.min(foamRate, GMAX - t2);
    }
  ...
  if (tot > 0) {
    ...
    document.getElementById("rv").textContent =
      `${Math.round(br * 100)}%:${Math.round(fr * 100)}%`;

    const score = calcScore(beer, foam);
...
function draw() {
  ...
  if (remain <= 5 && started && !over) {
    ...
    cx.textAlign = "center";
    cx.fillText("⏰ " + remain.toFixed(1), W / 2, 34);
  }
...
function drawCan() {
  ...
  const rad = (cAng * Math.PI) / 180;
  const offsetX = Math.sin(rad) * 8;
  const offsetY = -Math.cos(rad) * 4;

  cx.translate(cx2 + offsetX, cy2 + offsetY);
  cx.rotate(rad);
  ...
  cx.fillStyle = "rgba(255,245,190,0.9)";
  cx.beginPath();
  cx.ellipse(0, -ch - 2, 4, 2.5, rad, 0, Math.PI * 2);
  cx.fill();
...
function drawStream() {
  ...
  const cy2 = 130;
  const ch = 60;
  const topX = cx2 + Math.sin(rad) * ch;
  const topY = cy2 - Math.cos(rad) * ch;
  const sx = topX + Math.cos(rad) * 14;
  const sy = topY + Math.sin(rad) * 14;
  ...
  cx.strokeStyle = `rgba(245,210,120,${peff * 0.7})`;
  cx.lineWidth = 4 + peff * 4;
  cx.lineCap = "round";

  cx.beginPath();
  cx.moveTo(sx, sy);
  cx.bezierCurveTo(sx + 16, sy + 24, ex - 14, ey - 25, ex, ey);
  cx.stroke();

  cx.fillStyle = `rgba(255,245,190,${peff * 0.9})`;
  cx.beginPath();
  cx.arc(sx, sy, 3 + peff * 2, 0, Math.PI * 2);
  cx.fill();
